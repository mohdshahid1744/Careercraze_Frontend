...
function Otp() {
  ...
  const recruiter = location.state || { name: "", email: "", mobile: "", password: "",companyEmail:"",companyName:"" };
  ...
  const handleResendOtp = async () => {
    setTimer(60);
    try {
      const response = await axiosRecruiterInstance.post('/recruiter/recResendOTP', { email: recruiter.email });
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      const response = await axiosRecruiterInstance.patch('/recruiter/recVerifyotp', {
        otp: enteredOtp,
